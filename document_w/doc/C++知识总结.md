...
- shared_ptr：实现共享式拥有概念。多个智能指针可以指向相同对象，该对象和其相关资源会在“最后一个引用被销毁”时候释放。从名字share就可以看出了资源可以被多个指针共享，它使用计数机制来表明资源被几个指针共享
    ...
    当两个对象相互使用一个shared_ptr成员变量指向对方，会造成循环引用，使得引用计数失效，从而导致内存泄漏
...
- const_cast：去除指向常数对象的指针或引用的常量性，其去除常量性的对象必须为指针或引用

- static_cast：各种隐式转换，各种数据类型的转换，类层次中基类和派生类之间指针或引用的转化。空指针转换成目标类型的空指针，把任何类型的表达式转换成void类型

    `static_cast<类型说明符> (变量表达式)：static_cat<double> (a);a为int型`

- dynamic_cast：动态类型转换，只能用于含虚函数的类；在运行期间完成，成功返回指向类的指针或引用，失败返回NULL，比static安全

- reinterpret_cast：改变指针或引用的类型、将指针或引用转换为一个足够长度的整形、将整型转换为指针或引用类型

### C/C++中指针和引用的区别

指针：是一个变量，其值为另一个变量的地址，即内存地址

引用：一个变量的别名，对引用的操作与对变量的操作完全一样
...
- 指针有多级，但引用只有一级
- 指针使用++表示指向下一个空间，引用++表示对象值加1
- 引用没有const，指针有const；（指针有“指针常量”即int * const a,但是引用没有int& const a，不过引用有“常引用”即const int &a = 1）

### 堆和栈的区别，堆和栈最大是多大

- 申请方式不同：堆由程序员动态申请，new/malloc；栈由系统在编译器需要的时候分配，不需要的时候自动释放
- 管理方式不同：堆由程序员自己管理，delete/free；栈由系统管理
- 空间大小不同：堆是不连续的内存区域，系统用链表管理存储空闲内存地址，所以堆由低到高扩展的数据结构，受限于计算机系统有效虚拟内存，一般32位系统下可达到2.9G的大小；栈是高地址向低地址扩展的结构，栈顶和最大容量系统预先设定好，通常为1M，也有2M的，可以修改
- 申请后系统响应不同：只要栈的空间大于所空间，系统就为程序提供内存，否则异常提示栈溢出；堆在申请内存时，系统会遍历记录空闲内存地址的链表，寻找第一个空间大于所申请空间的堆结点，然后将该节点从空闲链表删除，并将该节点空间分配给程序
- 分配效率不同：栈由系统管理，分配专门的寄存器存放栈的地址，有专门的指令执行出入栈，因此效率较高；堆由函数库提供支持，机制复杂，效率较低             
- 存储内容：栈首先压入主函数中下条指令的地址，然后入栈实参，然后是被调函数的局部变量；堆通常在头部用一个字节存放大小，具体内容由程序员指定

### C/C++内存布局

- 栈区：系统自动分配与释放，存放运行时函数的返回地址，实参，局部变量等
- 堆区：一般有程序员自动分配，分配方式遍历存放空闲内存地址的链表，会产生内存碎片
- 静态全局区：存放全局变量，静态变量，常量
    - data段：存放已初始化且初值非0的全局变量和静态局部变量，编译时确定
    - bss段：存放未初始化或初值为0的全局变量和静态局部变量，由链接器获取内存
- 常量区：存放字符串常量，程序结束后由系统释放
- 代码段（text）：存放函数体的二进制代码。编译时确定

### 什么是野指针

指向一个**已删除的对象或者未经申请访问受限内存区域的指针（指向随机空间，不受控制**），不能通过判断是否为NULL来避免

- 与垂悬指针的区别：垂悬指针指曾指向一个对象，该对象被销毁，但是指针未置空
- 产生原因：定义时未初始化，释放的时候未置空，指针操作超过变量作用域（返回指向栈内存的指针或引用）

### 函数指针和函数指针

- 函数指针定义：**指向函数的指针变量**

    首先，函数指针是一个指针变量，该变量指向一个具体的函数；C在编译的时候，每一个函数都有一个入口地址，该入口地址就是函数指针所指向的地址
...
- 将可能会被继承的父类的析构函数设置为虚函数，可以保证我们在new一个子对象的时候，可以用父类指针指向子对象，释放父类指针的时候可以释放掉子类的空间，防止内存泄露

- C++默认的析构函数不是虚函数，因为虚函数需要额外的虚函数表和虚函数表指针，会占用额外的内存，对于不会被继承的类而言，如果析构函数是虚函数就会浪费内存

### define 和 const 的区别

- 编译器处理方式不同：define宏是在预处理阶段展开，cosnt是在编译运行阶段使用
- 类型和安全检查不同：define宏没有类型检查，仅仅是展开，const在编译阶段会执行类型检查
- 存储方式不同：define宏仅仅是字符串展开，有多少地方使用就展开多少次，不会分配内存；const在内存中分配内存
- const只给出对应的内存地址，在运行过程中只有一份拷贝，（全局的只读变量，存在静态区），而#define定义的常量在内存中有若干个拷贝，因此const可以节省空间，避免不必要的内存分配
- 提高了效率。 编译器通常不为普通const常量分配存储空间，而是将它们保存在符号表中，这使得它成为一个编译期间的常量，没有了存储与读内存的操作，使得它的效率也很高
- define宏只做替换，不做计算不做表达式求解

>const数据成员只在某个对象生存期内是常量，而对于整个类而言却是可变的，因为类可以创建多个对象，不同的对象其const数据成员的值可以不同。
>
>不能再类声明中初始化const数据成员，只能在类构造函数的初始化列表中进行

### C语言文件的编译和执行的四个阶段

将c程序转换成硬件上可运行的程序（可执行代码）

- 编译：把文本形式源代码翻译成机器语言形式的目标文件
- 链接：把目标文件、操作系统的启动代码和用到的库文件进行组织形成可执行代码

总共分为四个阶段：

- 预处理：根据文件中的预处理指令来修改文件内容（宏定义，条件编译指令，头文件等）
- 编译、优化阶段：通过词法、语法分析，确认符合规则之后，将其翻译成中间代码或汇编代码
    - 对代码优化：删除公共表达式，循环优化、无用赋值删除，已知量合并
    - 同硬件结构相关，考虑如何利用机器的各个硬件寄存器存放的值来减少对内存的访问次数，如何根据硬件执行指令的特点（流水线、RISC、CISC等）调整目标代码
- 汇编：汇编语言代码翻译成目标机器指令，得到目标文件（至少代码段+数据段）
- 链接：将有关的目标文件彼此连接，使得所有的目标文件称为一个能够装入操作系统执行的整体（可执行代码）
...
- 申请的内存位置不同：malloc从堆上动态分配内存，new从自由存储区上分配内存（自由存储区可以是堆也可以是静态存储区，看new怎么实现）
- 返回时类型安全性不同：malloc分配成功返回void*，要强制转换成我们想要的类型；new成功返回对象类型的指针或引用，更加安全
- 内存分配失败的返回值：malloc失败返回NULL；new失败的时候抛出bac_alloc异常
- 是否要指定内存大小：malloc需要显式指定所需内存大小；new无需指定，编译器会根据对象类型自行计算
- 是否调用构造函数/析构函数：malloc不会调用；new会调用构造函数，delete会调用析构函数
- 对数组的处理不同：malloc需要手动指定数组的大小`(int *) malloc(sizeof(int)*10)`;new会提供new[]来处理数组，分别调用构造函数去初始化每一个数组元素
- 是否可以被重载：malloc不可以重载，new可以重载



### malloc和free的底层实现

[malloc和free的底层实现]( https://blog.csdn.net/c1s2p3/article/details/50522185?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.channel_param&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.channel_param)

Linux维护一个break指针，这个指针指向堆空间的某个地址，从堆起始地址到break之间的地址空间为映射好的，可以供进程访问，而从break往上，是未映射的地址空间。malloc分配内存就是从break开始往上的。
...
C中的struct是对不同数据类型的集合；C++中是把数据类型和相关变量方法封装起来

C++中struct和class的区别

- 内部成员的默认访问控制属性不一样：struct默认为public。class默认为private
- 继承方式默认访问控制属性不一样：struct默认是public；class默认是private
- class还可以用来定义模板参数，但struct不可以

### 数组和指针的主要区别

- 指针保存数据的地址，数组保存数据
- 指针间接访问数据，首先获得指针的内容，然后将其作为地址，从该地址中获取数据；而指针则是直接访问数据
- 指针通常用于动态的数据结构，指向的内存区域大小可变，同时除非指向常量字符串之外，内容不可修改；数组通常用于固定数目并且数据类型相同的元素，地址和容量在生命周期都不会变，只有数组的内容可变
- 计算容量的不同，sizeof指针只能是4，2，8；但是sizeof数组是整个数组的容量

### 为什么析构函数必须是虚函数，为什么C++默认析构函数不是虚函数
...
- 构造函数在创建对象时完成对对象属性的一些初始化操作，**给对象一个标识符，为对象数据成员开辟内存空间，完成数据对象成员的初始化**，在没有构造函数的情况下会自动生成一个**默认构造函数，完成给对象标识符，为数据成员开辟内存空间的功能。**

    在创建对象时自动执行，函数名与类名相同，没有返回值，也没有返回值类型，不能显式调用，可带参数，用初始化列表进行赋值初始化；可重载

- 析构函数：在对象被撤销时自动调用，用于一些清理工作，例如释放new申请的空间。函数名与类名相同，前面加上~，没有返回类型，不指定参数，因此析构函数只有一个不能重载；**默认的析构函数只能释放普通数据成员所占据的空间，不能释放new/malloc分配的空间**

### const的作用

常类型是指用类型修饰符const说明的类型，一般常类型的变量或对象的值是不能被更新的。

const的作用：

- 定义const变量，表示只读
- const是在编译阶段起作用，具有类型检查的作用
- 可以作为函数参数，防止意外的修改，增加程序的健壮性
- 可以节省空间，避免不必要的内存分配
- 提高效率，编译器通常部位普通const常量分配内存空间，而是将他们保存在符号表中，称为编译期间的常量，没有了存储和读内存的操作

如何使用const

- 修饰常用数据类型（指针，对象，基本数据类型等）
- 用于函数，参数传递，返回值
- 用在类中，声明然后用初始化列表定义，用在类中函数表示不能修改类中变量
- 定义const对象，只能调用const修饰的函数

### vector的底层实现

vector底层实现的是维持一个动态数组，里面有一个指针指向一片连续的内存空间，当空间装不下的时候就会自动申请一片更大的内存空间（空间配置器），然后将原来的数据一一拷贝到新的空间，最后释放旧的空间。当删除的时候空间并不会被释放只是清空了里面的数据。另外vector维护的是一个连续的线性空间，因此支持随机存取，同时当空间不足时，在新的地方开辟一个2倍大的空间，然后拷贝元素，最后释放旧空间，所以对vector的任何操作，一旦引起空间重新分配，指向vector的迭代器就失效了。核心三步是：**开辟新空间，拷贝，释放原空间**

由于动态增长会引起**重新分配内存空间、拷贝原空间、释放原空间**，这些过程会降低程序效率。因此，可以使用reserve(n)预先分配一块较大的指定大小的内存空间，这样当指定大小的内存空间未使用完时，是不会重新分配内存空间的，这样便提升了效率。只有当n>capacity()时，调用reserve(n)才会改变vector容量。

- 空的vector对象，size()和capacity()都为0
- 当空间大小不足时，新分配的空间大小为原空间大小的2倍。
- 使用reserve()预先分配一块内存后，在空间未满的情况下，不会引起重新分配，从而提升了效率。
- 当reserve()分配的空间比原空间小时，是不会引起重新分配的。比如此时capacity是20，只reserve（15）是不会重新分配的
-  resize()函数只改变容器的元素数目，未改变容器大小。

### 浅拷贝与深拷贝

在对象拷贝过程中，如果没有定义拷贝构造函数，系统就会调用一个默认的拷贝构造函数，默认拷贝构造函数对于基本类型的成变量，按字节复制，对于类类型成员变量，调用相应类型的拷贝构造函数。但是对应于有指针成员在的话，默认的拷贝构造函数就会出现问题，会使得**多个对象共用同一块资源，同一块资源最后会释放多次，导致程序崩溃或者内存泄漏，这就是浅拷贝**。

深拷贝：每个对象共同拥有自己的资源，必须显式地提供拷贝构造函数和赋值运算符，**指针和指针指向地内容都进行拷贝**，拷贝后地指针指向不同地地址深拷贝：**如果在复制这个对象的时候为新对象制作了外部对象的独立复制，就是深拷贝**
...
==拷贝构造函数是在对象被创建的时候调用的，赋值函数只能被已存在了的对象调用==

**另一个解决办法就是通过智能指针shared_ptr，来解决浅拷贝的带来的崩溃或者内存泄漏问题**

###  在c++程序中调用被C编译器编译后的函数，为什么要加extern“C”

因为C＋＋和C是两种完全不同的编译链接处理方式，所以如果直接在C＋＋里面调用C函数，这样链接起来是通不过的，会报链接错误，找不到函数体，所以要在 C＋＋文件里面显示声明以下一些函数是C写的，要用C的方式来处理。

在C++中支持函数重载，C不支持。函数被C++编译后在符号库中的名字与C语言的不同。例如一个函数full在C编译下名字为_full，而在C++编译器下就可能是__full_int_int_之类的名字，通过extern告诉编译器去寻找C编译下的名字，从而解决名字匹配的问题。

cpu  海思优化 gpu（cpu）

### C++如何实现多态（讲明多态的三个条件，实现原理)

三个条件：要有继承关系、要有虚函数重写、要有基类指针指向子类对象

对于每一个有虚函数的基类，都会有一张虚函数表，虚函数表中的每一个表项对应着每一个虚函数的地址，在子类继承父类的时候，也会将虚函数表继承过来，同时如果子类会重写父类的虚函数，那么就会在子类的虚函数表中更新对应虚函数的地址。在创建一个对象的时候，分配的内存空间的开始位置存放着一个虚表指针，指向虚函数表，当实例调用相应的方法时就会用虚函数指针得到子类的虚函数地址，调用子类的方法。

### 构造函数为什么一般不定义为虚函数

**从存储空间看**：虚函数相应一个指向vtable虚函数表的指针，而这个指向vtable的指针事实上是存储在对象的内存空间的。如果此时构造函数是虚的，就须要通过vtable来调用，但是对象还没有实例化，也就是内存空间还没有vtable，所以构造函数不能是虚函数

**从实用角度看**：虚函数主要使用在信息不全的情况下，能使得重载的函数得到相应的调用。虚函数的作用在于通过父类的指针或者引用来调用它的时候可以变成调用子类的那个成员函数。而构造函数是在创建对象时自己主动调用的，不可能通过父类的指针或者引用去调用，因此构造函数没必要是虚函数

**从实现上看**：在调用构造函数时还不能确定对象的真实类型（由于子类会调父类的构造函数）；并且构造函数的作用是提供初始化，在对象生命期仅仅运行一次，不是对象的动态行为，也没有必要成为虚函数。

### C++11新特性

- 增加范围型for循环
- 增加auto：类型推导，不能用作传参
- 增加nullptr：为了替代NULL传统C++将NULL和0视为同一种东西，有的定义为0，有的定义为（（void*）0），C++不允许将 void*隐式转换成其他类型，这在重载的时候会出现问题
- Lamba表达式：匿名函数，没有名字的函数`[capture](params) opt->ret{body};`
- 新增容器array：保存在栈中，编译时创建固定大小数组
- 右值引用：左值表示的对象在内存中的地址，右值表示的是对象的内容或者值
